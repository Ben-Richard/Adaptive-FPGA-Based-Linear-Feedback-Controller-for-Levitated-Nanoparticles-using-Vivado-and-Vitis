...
Workflow Overview
Python (make_arb_sines.py) → Generate multi-tone input waveform.
Python (get_filter_coeffs.py) → Generate fixed-point coefficients for Verilog.
MATLAB (test_filter.m) → Design and verify bandpass filter response.
Verilog (Vivado project) → Implement and simulate the Controller.
Python (analyse_fft.py) → Analyse simulation results (FFT, per-tone gain).
(Optional) Deploy waveform to Red Pitaya hardware for real measurements.
Requirements
